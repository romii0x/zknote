...
describe('Client-side Encryption', () => {
  ...
  test('encrypts and decrypts in browser, never sending plaintext to server', async () => {
    ...
    expect(window.fetch).toHaveBeenCalledWith('/api/shout', expect.objectContaining({ body: encrypted }));
    expect(window.fetch).not.toHaveBeenCalledWith('/api/shout', expect.objectContaining({ body: message }));
  });
}); 