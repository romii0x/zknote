  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        return reply
          .status(500)
          .send(await errorResponse(500, "Failed to create message"));
      }
    },
  );
    ...
    async (request, reply) => {
      ...
      try {
        ...
        if (res.rowCount === 0) {
          return reply
            .status(404)
            .send(await errorResponse(404, "Message not found"));
        }
        ...
        if (msg.expires && msg.expires < Date.now()) {
          ...
          return reply
            .status(410)
            .send(await errorResponse(410, "Message expired"));
        }
      ...
      if (!deleted) {
        return reply
          ...
          .send(
            await errorResponse(
              404,
              "Message not found or invalid delete token",
            ),
          );
      }
